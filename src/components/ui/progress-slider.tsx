...
interface Item {
    ...
    id: string // Added unique ID for keys
}
            ...
            <div className="transition-all duration-150 delay-300 ease-in-out">
                <div className="relative flex flex-col" ref={itemsRef}>
                    ...
                    {items.map((item) => (
                        <Transition
                            key={item.id}
                            show={active === items.indexOf(item)}
            ...
            <div className="max-w-xs sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                ...
                {items.map((item) => (
                    <button
                        ...
                        onClick={() => { setActive(items.indexOf(item)); setProgress(0) }}
                    >
                        <span className={`text-center flex flex-col items-center ${active === items.indexOf(item) ? '' : 'opacity-50 group-hover:opacity-100 group-focus:opacity-100 transition-opacity'}`}>
                            ...
                            <span 
                                ...
                                role="progressbar" 
                                aria-valuenow={active === items.indexOf(item) ? progress : 0}
                                aria-valuemin={0}
                                aria-valuemax={100}
                                tabIndex={0}
                            >
                                <span 
                                    ...
                                    style={{ width: active === items.indexOf(item) ? `${progress}%` : '0%' }} 
                                />
                            </span>
                        </span>
                    </button>
